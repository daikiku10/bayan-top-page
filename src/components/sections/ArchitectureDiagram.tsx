...
export default function ArchitectureDiagram({
  ...
  const needsScroll = columnCount > 2;

  return (
    <div>
      {needsScroll && (
        <p className="mb-2 text-xs text-stone-400 sm:hidden">
          → 横にスクロールできます
        </p>
      )}
      <div className="overflow-x-auto">
        <div
          ref={containerRef}
          className="relative"
          style={{ minWidth: needsScroll ? "560px" : undefined }}
        >
          <svg
          ...
          >
            ...
            {edges.map((edge) => {
              ...
              const sameColumn =
                nodeById.get(edge.from)?.column ===
                nodeById.get(edge.to)?.column;
